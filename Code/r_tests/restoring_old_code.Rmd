...
# A one dimensional example to start. 
...
# Vector to store selected 'x' values
selected_x <- numeric()
...
for (iteration in 1:max_iterations) {
  ...
  # Removes previously selected 'x' values from consideration
  available_x <- setdiff(x_new, selected_x)
  ...
  min_lower_bound_idx <- which.min(lower_confidence_bound)
  selected_x_value <- available_x[min_lower_bound_idx]
  selected_x <- c(selected_x, selected_x_value)
