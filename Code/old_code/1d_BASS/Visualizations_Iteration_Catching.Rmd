---
title: Visualizations Iteration Catching Algorithm
...
# Controls how many points are evaluated in the BASS prediction
n1 <- 1000
...
while (iteration <= max_iterations) {
  ...
  # Finds the 'x' value that minimizes the lower confidence bound and is not 
  # already present in 'evaluations'
  min_index <- which.min(lower_confidence_bound)
  x_min <- x_new[min_index]
  
  # Checks if the 'x' value is not already present in 'evaluations'
  if (!any(evaluations[, "x"] == x_min)) {
    # If the point is a repeat, find another random point. 
    x_random <- maximinLHS(1,1)[,1]
    evaluations <- rbind(evaluations,
                         c(x_random, f(x_random)))
    
    iteration <- iteration + 1  # Increment the iteration count
  }
  print(iteration)
}
```
