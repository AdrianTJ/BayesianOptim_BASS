...
$\boldsymbol{\theta}$ denotes the vector of hyperparameters. Let  $\mathbf{y} = (y_1, \ldots, y_n)^T$ and let $K_{\boldsymbol{\theta}} \in \mathbb{R}^{n \times n}$ denote the kernel matrix with entries $(K_{\boldsymbol{\theta}})_{ij} = k_{\boldsymbol{\theta}}(x_i, x_j)$. 
Under the Gaussian process prior and the additive noise model, the marginal distribution of the observations is}
\[ \mathbf{y} \mid X, \boldsymbol{\theta} \sim \mathcal{N}\!\left(\mathbf{0},\, K_{\boldsymbol{\theta}} + \sigma_n^2 I\right), \]
...
\[ \log p(\mathbf{y} \mid X, \boldsymbol{\theta}) = -\frac{1}{2} 
\mathbf{y}^T C_{\boldsymbol{\theta}}^{-1} \mathbf{y} - \frac{1}{2} 
...
\begin{equation}
	...
	\left[ \frac{1}{2} \mathbf{y}^T C_{\boldsymbol{\theta}}^{-1} 
	\mathbf{y} + \frac{1}{2} \log \det C_{\boldsymbol{\theta}} \right],
	\label{eq:mll}
\end{equation}
