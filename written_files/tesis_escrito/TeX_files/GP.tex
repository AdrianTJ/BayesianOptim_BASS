...
\begin{proof}
	...
	\green{To verify that $\pi$ is a probability distribution: since
		...
		$j \in \mathcal{S}$. Furthermore, since $\sum_{j \in \mathcal{S}} p_{i,j}^{(n)} = 1$ for all $n$, taking the limit yields $\sum_{j \in \mathcal{S}} \pi_j = 1$. Hence $\pi$ is a valid probability distribution on $\mathcal{S}$.}
...
\[ \text{cov}(\boldsymbol{y}) = \sum \boldsymbol{y} = K(\boldsymbol{X}, \boldsymbol{X}) + \sigma^2 I, \]
