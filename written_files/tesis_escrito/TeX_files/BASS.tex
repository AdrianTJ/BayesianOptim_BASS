...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		lab/.style={align=left, font=\footnotesize},
		arr/.style={-{Stealth[length=2.6mm]}, thick}]
		\node[box] (cart) {CART \\ \footnotesize decision trees \cite{breiman2017classification}};
		\node[box, below=of cart] (mars) {MARS \\ \footnotesize adaptive regression splines \cite{friedman1991multivariate}};
		\node[box, below=of mars] (bmars) {BMARS \\ \footnotesize Bayesian MARS \cite{denison1998bayesian}};
		\node[box, below=of bmars] (bass) {BASS \\ \footnotesize adaptive spline surfaces \cite{francom2020bass}};
		\draw[arr] (cart) -- (mars) node[midway, right=2mm, lab] {steps become hinges:\\continuous fits};
		\draw[arr] (mars) -- (bmars) node[midway, right=2mm, lab] {structure becomes random:\\priors and a posterior};
		\draw[arr] (bmars) -- (bass) node[midway, right=2mm, lab] {efficient sampling, tempering,\\native categorical factors};
	\end{tikzpicture}
	\caption{The theoretical basis for BASS: each model in the lineage keeps its predecessor's structure and relaxes one of its commitments.}
...
\begin{figure}[h]
	\includegraphics[width=11cm]{Figures/decision_tree.png}
	\centering
	\caption{A regression tree for the house-pricing example: two splits, one of them on a categorical variable, with leaf predictions equal to the mean response of the training points in each leaf.}
...
$$
\begin{figure}[h]
	\includegraphics[width=11cm]{Figures/reflected_pair.png}
	\centering
	\caption{\green{The constructed reflected pair: both hinge functions share the knot $t = x_{ij}$, marked on the axis; each is zero on the side where its partner is active.}}
...
\begin{figure}[h]
	\includegraphics[width=12cm]{Figures/hinge_collection.png}
	\centering
	\caption{\green{The collection of candidate reflected pairs for one predictor $X_j$: one pair per observed value, each knot $x_{1,j}, x_{2,j}, \ldots, x_{N,j}$ marked at its vertex.}}
	\label{hinges}
\end{figure}
...
$$
\green{where the candidates for the new term are built by taking a basis function $h_\ell$ already in $\mathcal{M}$ (possibly the constant $h_0 = 1$), a predictor $X_j$ that $h_\ell$ does not already use, and a knot $t$ at one of that predictor's observed values $x_{ij}$. In the running two-predictor example, if $\mathcal{M}$ contains the term $(X_1 - 0.3)_+$, one candidate multiplies it by the reflected pair of $X_2$ at the knot $0.7$, yielding the two interaction candidates $(X_1 - 0.3)_+ (X_2 - 0.7)_+$ and $(X_1 - 0.3)_+ (0.7 - X_2)_+$; whichever pairing reduces the training error most is the one that enters the model.}
...
\green{To ground this, return to the two-predictor setting of the MARS example, $m = 2$ with predictors $x_1$ and $x_2$. A basis function can involve $x_1$ alone, $x_2$ alone, or both, so there are $2^2 - 1 = 3$ types. Writing $\ast$ as a placeholder for a knot location (whose value does not affect the type), the three shapes are $[\pm (x_1 - \ast)]$ (type 1), $[\pm (x_2 - \ast)]$ (type 2), and $[\pm (x_1 - \ast)][\pm (x_2 - \ast)]$ (type 3). The two example terms $0.5\,(x_1 - 0.3)_+$ and $1.2\,(0.7 - x_2)_+$ from the previous section are then of type 1 and type 2 respectively, and any basis splitting only on $x_2$, whatever its knot or sign, has type 2. The type records which predictors a basis touches and nothing else. Figure~\ref{fig:bmars_types} shows one representative of each type as a surface over the unit square: the type 1 basis varies only along $x_1$, the type 2 basis only along $x_2$, and the type 3 basis is nonzero exactly where both of its factors are active, which is what an interaction means geometrically.}

\begin{figure}[h]
	\includegraphics[width=\textwidth]{Figures/bmars_types.png}
	\centering
	\caption{\green{The three basis types of the two-predictor example. Left to right: a type 1 basis $(x_1 - 0.3)_+$, flat in $x_2$; a type 2 basis $(0.7 - x_2)_+$, flat in $x_1$; and a type 3 interaction $(x_1 - 0.3)_+ (0.7 - x_2)_+$, nonzero only where both factors are positive.}}
	\label{fig:bmars_types}
\end{figure}
...
The only difference between this setup and that of MARS and BMARS is the inclusion of the constant $g_{km}$ in each element of the tensor product. \green{This term is the constant $g_{km} = [(s_{km}+1)/2 - s_{km} t_{km}]^{\alpha}$, chosen so that each basis function attains a maximum of one \cite[\S 2]{francom2020bass}.} \green{Here,} $s_{km} \in \{-1,1\}$ is referred to as the sign, $t_{km}\in [0,1]$ is a knot, $v_{km}$ selects a variable, $K_m$ is the degree of interaction, and $\alpha$ determines the degree of the polynomial splines. This normalizes the basis functions so that the basis coefficients $a_1, \ldots, a_M$ are on the same scale, making computations more stable.
...
\begin{align}
	M \mid \lambda &\sim \text{Poisson}(\lambda) \text{ truncated to } \{0, 1, \ldots, M_{\max}\}, \qquad \lambda \sim \text{Gamma}(h_1, h_2), \label{eq:bass_prior_M}\\
	\boldsymbol{a} \mid \sigma^2, \tau, \boldsymbol{B} &\sim \mathcal{N}\!\left(\boldsymbol{0}, \; \frac{\sigma^2}{\tau} \left(\boldsymbol{B}^T \boldsymbol{B}\right)^{-1}\right), \qquad \tau \sim \text{Gamma}(a_\tau, b_\tau), \label{eq:bass_prior_a}\\
	\sigma^2 &\sim \text{InvGamma}(g_1, g_2), \label{eq:bass_prior_sigma}
\end{align}
\green{where $\boldsymbol{B}$ is the $n \times (M+1)$ matrix of the basis functions evaluated at the data, so that the coefficient prior is Zellner's $g$-prior. The default settings in \cite{francom2020bass} are $g_1 = g_2 = 0$, which reduces the prior on $\sigma^2$ to the non-informative $p(\sigma^2) \propto 1/\sigma^2$, and $a_\tau = 1/2$, $b_\tau = 2/n$, which makes the coefficient prior the Zellner--Siow Cauchy prior; the truncation point $M_{\max}$ bounds computational cost rather than fighting overfitting, and $h_1 = h_2 = 10$ keeps the expected number of basis functions small. The structural components of each basis (the interaction degrees, signs, knots, and variables, with variables never repeated inside one basis, and for categorical factors the level subsets $S_{km}$) carry priors that are uniform over a constrained space: the constraint requires every basis function to have more than $b$ non-zero values at the data, which is the spline analogue of demanding a minimum number of points per partition in recursive partitioning.}

\green{The posterior over the full configuration,}
\begin{equation}
	p\!\left(\boldsymbol{\theta}, \lambda, \tau \;\middle|\; \mathcal{D}\right) \; \propto \; \text{likelihood} \times \text{priors}, \qquad \boldsymbol{\theta} = \{\sigma^2, M, \boldsymbol{a}, \boldsymbol{K}, \boldsymbol{s}, \boldsymbol{t}, \boldsymbol{v}\}, \label{eq:bass_posterior}
\end{equation}
\green{is the distribution the reversible-jump sampler targets, where $\boldsymbol{\theta}$ collects the parameters exactly as in \cite[\S 2]{francom2020bass}. Each iteration proposes adding, deleting, or changing one basis function; proposals favour the variables and interaction degrees already present in the model, which is the efficiency improvement BASS inherits, and quantities such as $\boldsymbol{B}^T\boldsymbol{B}$ are updated incrementally rather than recomputed, since each move changes the basis set only slightly. This is where Bayes enters the model of equations \eqref{eq:bass_lik} through \eqref{eq:bass_basis}: every structural decision that MARS makes greedily and once is here a random quantity averaged over its posterior, and the fitted object the sampler returns is the collection of posterior draws over entire spline surfaces that the uncertainty estimation of the next section relies on.}
