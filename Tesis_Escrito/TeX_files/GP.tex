...
which we denote as $p_{n,k}^{(k)}$. Because we can define the individual probabilities of transitioning from one step to another after $k$ steps, it stands to reason that there would be a transition matrix to describe this behaviour.
...
\begin{proof}
	...
	\begin{align*} 
		...
		&= \sum_{k \in \mathcal{S}} \frac{\mathbb{P}(X_m = s_j, X_r = s_k, X_0 = s_i)}{\mathbb{P}(X_0 = s_i)} \\
		&= \sum_{k \in \mathcal{S}} \mathbb{P}(X_m = s_j | X_r = s_k) \mathbb{P}(X_r = s_k | X_0 = s_i) \\
		&= \sum_{k \in \mathcal{S}} p_{i,k}^{(r)} \, p_{k,j}^{(m-r)}. 
	\end{align*}
\end{proof}
